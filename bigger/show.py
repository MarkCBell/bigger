...
def interpolate(A, B, C, r, s):
    ...
    # Return X := rB + (1-r)A and
    # Y := sB + (1-s)C
    return (r*B[0] + (1-r)*A[0], r*B[1] + (1-r)*A[1]), (s*B[0] + (1-s)*C[0], s*B[1] + (1-s)*C[1])
...
def layout_triangulation(triangulation: 'bigger.Triangulation[Edge]', edges: List[Edge], w: int = 100, h: int = 100) -> Dict[Triangle, Tuple[float, float]]:
    ...
    for index, component in enumerate(components):
        ...
        vertices = [
            (
                dx * (index % p) + dx / 2 + r * sin(2 * pi * (i + 0.5) / num_outside),
                dy * int(index / p) + dy / 2 + r * cos(2 * pi * (i + 0.5) / num_outside)
            )
        ...
        while to_extend:
            ...
            if y in interior: to_extend.append((other, y))
            if z in interior: to_extend.append((other, z))
...
def show(item: Union['bigger.Triangulation', 'bigger.Lamination'], edges: List[Edge], **options) -> Image:
    ...
    if 'w' not in options: options['w'] = 400
    if 'h' not in options: options['h'] = 400
